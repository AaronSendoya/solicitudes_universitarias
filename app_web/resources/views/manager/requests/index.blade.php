                    ...
                    <table class="w-full text-left text-sm text-gray-600 dark:text-gray-300">
                        <thead class="bg-gray-50 dark:bg-gray-700/50 text-xs uppercase text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 font-semibold">Técnico Asignado</th>
                            ...
                            @forelse($requests as $req)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors {{ $req->priority === 'Crítica' && $req->status !== 'Cerrado' && $req->status !== 'Resuelto' ? 'bg-red-50/50 dark:bg-red-900/10' : '' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($req->assignedTo)
                                            <div class="flex items-center">
                                                <div class="w-6 h-6 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-xs font-bold mr-2">
                                                    {{ substr($req->assignedTo->name, 0, 1) }}
                                                </div>
                                                <span class="text-sm font-medium">{{ $req->assignedTo->name }}</span>
                                            </div>
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <a href="{{ route('manager.requests.show', $req->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                            Gestionar
                                        </a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <p class="mt-1 text-sm">No hay solicitudes registradas en el sistema en este momento.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
